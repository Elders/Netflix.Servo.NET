...
namespace Java.Util.Concurrent
{
	...
	public class ExecutorCompletionService<T> : ICompletionService<T> //JDK_1_6
	{
		...
		{
			if (executor == null)
				throw new ArgumentNullException(nameof(executor), "Executor cannot be null.");
			if (completionQueue == null)
				throw new ArgumentNullException(nameof(completionQueue), "Completion Queue cannot be null.");
			_executor = executor;
