...
namespace Java.Util.Concurrent
{
    ...
    public class ThreadPoolExecutor : AbstractExecutorService, IDisposable, IRecommendParallelism //BACKPORT_3_1
    {
        ...
        protected internal class Worker : ReentrantLock, IRunnable
        {
            ...
            /// </summary>
            internal volatile uint CompletedTasks;
        ...
        public TimeSpan KeepAliveTime
        {
            set
            {
                if (value.Ticks < 0)
                {
                    throw new ArgumentOutOfRangeException(nameof(value), value, "Keep alive time must be greater than 0.");
                }
        ...
        public int CorePoolSize
        {
            ...
            {
                if (value < 0)
                    throw new ArgumentOutOfRangeException(nameof(value), value, "CorePoolSize cannot be less than 0");
                                  ...
                                  IThreadFactory threadFactory, IRejectedExecutionHandler handler)
        {
            if (corePoolSize < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(corePoolSize), corePoolSize,
                    "core pool size cannot be less than zero.");
            }
            if (maximumPoolSize <= 0)
            {
                throw new ArgumentOutOfRangeException(nameof(maximumPoolSize), maximumPoolSize,
                    "maximum pool size must be greater than zero");
            }
            if (maximumPoolSize < corePoolSize)
            {
                throw new ArgumentException("maximum pool size, " + maximumPoolSize +
                    " cannot be less than core pool size, " + corePoolSize + ".", nameof(maximumPoolSize));
            }
            if (keepAliveTime.Ticks < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(keepAliveTime), keepAliveTime,
